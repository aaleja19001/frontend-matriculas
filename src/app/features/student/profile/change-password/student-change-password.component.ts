      ...
      <div style="background: #FFFFFF; border: 1px solid #E2E8F0; border-radius: 1rem; padding: 2rem;">
        ...
        <form (ngSubmit)="onSubmit()" style="display: flex; flex-direction: column; gap: 1.5rem;">
          ...
          <div style="margin-top: 1rem; display: flex; justify-content: flex-end; gap: 1rem;">
            <button type="button" routerLink="/student/profile"
            ...
            <button type="submit" [disabled]="loading() || !currentPassword || !newPassword || !confirmPassword"
                    style="padding: 0.625rem 1.25rem; border-radius: 0.5rem; border: none; background: #2563EB; color: white; font-size: 0.875rem; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 0.5rem;">
  ...
  onSubmit() {
    ...
    this.authService.changePassword(this.currentPassword, this.newPassword).subscribe({
      next: () => {
        ...
        setTimeout(() => this.router.navigate(['/student/profile']), 2000);
      },
      error: (err) => {
        this.loading.set(false);
        this.message.set({ 
